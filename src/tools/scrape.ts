...
export const scrapeSchema = z.object({
  ...
  sticky_session:
    z.union([z.string(), z.unknown()]).optional().describe("Customer-facing sticky-session handle. Two separate /scrape calls that pass the same sticky_session value are pinned to the SAME exit IP and automatically share the cookies captured on the first call (see captured_cookies in the response), for the handle's TTL. Enables stateful multi-request flows — e.g. request 1 loads a page, request 2 fetches the captcha/image that appeared using request 1's IP + cookie jar. Requires a paid plan (growth tier or above). Alphanumeric, '-' and '_' only, max 128 chars."),
  sticky_session_ttl:
    z.union([z.number(), z.unknown()]).optional().describe("Sticky-session lifetime in seconds (300–7200, default 1800). The exit IP and captured cookies bound to sticky_session are held for this window, after which the handle resolves to a fresh IP/jar. Ignored unless sticky_session is set."),
  use_own_proxy:
